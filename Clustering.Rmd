---
title: "Clustering Positions in Basketball"
...
3. For each of the $K$ clusters, compute the cluster centroid. The $k^{th}$ cluster centroid is the vector of the p feature means for the observations in the $k^{th}$ cluster and finds the centroid of each cluster.
...
We want to use only numeric value rows for this classification process so we will create a data frame with such va.
...
Now let’s examine the correlation between each of these variable to determine which axis to use when constructing and evaluating our clusterplot.

The correlation plot between the different statistical categories will show if there’s anything interesting interactions among them. We will now call the corrpplot library to help visualize the correlation between each variable.
...
Here is the R code for generating the Elbow Method in order to find the optimal number of clusters.
First we will generate a set of random numbers to help with this simulation.
...
Now with compute and the sum of squares with each cluster from k=2 to k=10

```{r}
k.max <- 10
```

We will be able to perform k-means clustering on the data matrix Per100.
...
Now we can construct a plot of wss for k = 2 to k = 10.
...
To determine and visualize the optimal number of clusters we will use the following function
...
We will use the aggregate function to generate the cluster means.
...
More polished interior and midrange scorer, superb rebounder and focal point of the offense 
